...
export class OrderService {
  ...
  getPaginatedOrders(
    ...
    // console.log('getPaginatedOrders called');
  ...
  assignDeliveryAgent(
    ...
    return this.http.put(`${this.apiUrl}/assignDeliveryAgent`, null, {
      params: {
  ...
  changeOrderStatus(orderId: number, newStatus: OrderStatus): Observable<any> {
    return this.http.put(`${this.apiUrl}/changeStatus/${orderId}`, {
      newStatus,
    });
  }
  ...
  getShippingTypes(): Observable<ShippingType[]> {
    return of(
      Object.values(ShippingType).filter(
        (value) => typeof value === 'number'
      ) as ShippingType[]
    ).pipe(delay(1000));
  }
